...
export default class Paint extends Sheet {
    ...
    paintRange: PaintRange = {
        ...
        // 因为自动滚动选中后定时器问题会导致形参永远时初始值，这里改成对象指针解决此问题
        boundEndCol: 0,
        boundEndRow: 0
    }
    /**
    ...
    */
    public paint = () => {
        ...
        // 绘制table部分
        this.paintBody(startRowIndex, endRowIndex, startColIndex, endColIndex);
        // 冻结列头
        this.paintFrozenCol(startRowIndex, endRowIndex);
        // 冻结行头
        this.paintFrozenRow(startColIndex, endColIndex);
        // 头部列标A，B，C....
    ...
    private paintBody = (
        ...
        const paintedMap = {};
        const canvasContext = this.options.canvas.getContext();
    ...
    private paintTopOrder = (startColIndex: number, endColIndex: number, frozen?: boolean) => {
    ...
    private paintSelectedRange = () => {
        if (!this.selectedCell || !this.selectedRange.length) return;
        const scrollLeft = this.scrollBar.getHorizontal().scrollLeft;
     ...
     * 绘制冻结行
     */
    private paintFrozenRow = (startColIndex: number, endColIndex: number) => {
    ...
    private paintFrozenCol = (startRowIndex: number, endRowIndex: number) => {
    ...
    private paintCell = (cell: any, x: number, y: number) => {
        this.paintCellBorder(cell, x, y);
        this.paintCellBgColor(cell, x, y);
        this.paintCellValue(cell, x, y);
    }
    private paintCellBorder = (cell: any, x: number, y: number) => {
        const scrollLeft = this.scrollBar.getHorizontal().scrollLeft
        const scrollTop = this.scrollBar.getVertical().scrollTop
    ...
    private paintCellBgColor = (cell: any, x: number, y: number) => {
        const scrollLeft = this.scrollBar.getHorizontal().scrollLeft;
        const scrollTop = this.scrollBar.getVertical().scrollTop;
        const lineX = x !== undefined ? x : cell.x - scrollLeft;
        const lineY = y !== undefined ? y : cell.y - scrollTop;
        this.options.canvas.paintRect({
            ...
            width: cell.width,
            height: cell.height,
            fillStyle: cell?.style?.backgroundColor || '#FFFFFF'
        });
    }
    private paintCellValue = (cell: any, x: number, y: number) => {
        ...
        canvasContext.fillStyle = cell?.style?.color || '#000';
        if (style) {
            font = `
					${style?.fontStyle || 'normal'} ${style?.fontVariant || 'normal'} ${style?.fontWeight || 'normal'} ${pxToNum(style?.fontSize || FONT_SIZE)}px ${style?.fontFamily || FONT_FAMILY}
				`
        }
        ...
        const fontWidth = this.options.canvas.getContext().measureText(value).width;
